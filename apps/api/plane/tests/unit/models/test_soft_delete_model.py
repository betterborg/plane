...
SOFT_DELETE_DATABASE_ALIAS = "soft_delete_test"


@pytest.mark.unit
@pytest.mark.django_db(transaction=True, databases={"default", SOFT_DELETE_DATABASE_ALIAS})
class TestSoftDeleteModel:
    def test_committed_soft_delete_publishes_recursive_deletion_after_commit(self):
        project = ProjectFactory()
        project = type(project).all_objects.using(SOFT_DELETE_DATABASE_ALIAS).get(pk=project.pk)

        with patch("plane.db.mixins.soft_delete_related_objects.delay") as delay:
            with transaction.atomic(using=SOFT_DELETE_DATABASE_ALIAS):
                project.delete(using=SOFT_DELETE_DATABASE_ALIAS)
                delay.assert_not_called()

            delay.assert_called_once_with("db", "project", project.pk, using=SOFT_DELETE_DATABASE_ALIAS)
    ...
    def test_rolled_back_soft_delete_does_not_publish_recursive_deletion(self):
        ...
        project.refresh_from_db(using=SOFT_DELETE_DATABASE_ALIAS)
    ...
    def test_registers_named_robust_callback(self):
        ...
        on_commit.assert_called_once_with(callback, using=SOFT_DELETE_DATABASE_ALIAS, robust=True)
